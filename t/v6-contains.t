...
# $Id: v6-contains.t,v 1.2 2004/02/23 22:38:28 lem Exp $
...
ok(NetAddr::IP->new('::')->contains(NetAddr::IP->new('::')), 
   ":: contains itself");

for my $p (@yes_pairs)
{
    my $a = new NetAddr::IP $p->[0];
    my $b = new NetAddr::IP $p->[1];

    isa_ok($a, 'NetAddr::IP', "$p->[0]");
    isa_ok($b, 'NetAddr::IP', "$p->[1]");
